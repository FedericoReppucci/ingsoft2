...
sig IgnitedCar extends Car{
	...
	//an ignited car can't be parked
	no parkedIn/*useless?*/
}
...
fact{ no c1, c2 : Car | c1 != c2 and (some p : Person | p in c1.hasIn and p in c2.hasIn ) }

//a driver must be in a car to drive it
fact{no d : Driver | ( one c : Car | c in d.isDriving and d not in c.hasIn ) }

//no two sensor detect the same person
fact{ no s1, s2 : Sensor | s1 !=  s2 and ( some p : Person | p in s1.detectedPerson and p in s2.detectedPerson ) }
...
fact{ all c : Car | ( one d : Driver | c in d.hasParked implies one c.parkedIn) }
...
pred p{/* one d:Driver| #d.isDriving > 0 and some d:Driver | one  d.isDriving and ( some u:User | one u.isChargedFor ) and*/ ( one c : Car | c in StoppedCar)/*and some d.isDriving.displays*/}
run p for 4
...
//[R7.1]: When a car is stopped and the sensors in the car detect no one inside,
// if a user was being charged for the car the system stops charging him/her. 
fact{ all c : StoppedCar | no c.hasIn }
